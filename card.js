...
async function createCard(data, callback) {
  ...
  if (data.team) {
    try {
      ...
      if (fs.existsSync(teamPath)) {
        ...
        ctx.drawImage(teamImage, 185, 470, 100, 60); // Позиция для фото команды
  ...
  console.log('Card created at:', resultPath);

  // Проверка существования файла карточки
  if (fs.existsSync(resultPath)) {
    console.log('File exists:', resultPath);
    callback(resultPath);
  } else {
    console.error('File does not exist:', resultPath);
    callback(null);
  }
}
